...
const arr = [3,4,5,6,7,8,9,2]

function pari (arr){
  let numPar = arr.filter(function(num){
    return num %2 ===0;
  });
...
function sumRed (arr){
   sum = arr.reduce(function(numTot,numCorr){
    return numTot + numCorr;
  }, 0);
  return sum;
}

console.log(sumRed(arr))
...
function sumToArr(arr,n){
  sum = arr.map(function(num){
    return num +n;
  });
  return sum;
}

console.log(sumToArr(arr,3));
...
let words = ["Epicode", "is", "great","today","and","ever"];
...
function older (arr){
  ...
  arr.forEach(function(film) {
    if(parseInt(film.Year) < parseInt(older.Year)) {
      older = film;
    }
  });
...
function filt (movies){
  
  const recentMovies = movies.filter(movie => parseInt(movie.Year) > 2000);

console.log(recentMovies);
}

console.log(filt(movies));
...
const totalYears = sumYears(movies);
...
function findFilm(movies, imdbID) {
  const film = movies.find(movie => movie.imdbID === imdbID);
  return film ? film : console.log("Film not found.");
}
...
function findFilmIndex(movies, year) {
  let index = movies.findIndex(movie => parseInt(movie.Year) === year);
...
console.log(findFilmIndex(movies, 2010));
